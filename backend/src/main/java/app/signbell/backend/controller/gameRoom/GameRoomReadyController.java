...
/**
 * GameRoomReadyController는 WebSocket을 통해
 * 게임방 참가자의 '준비 상태 변경' 요청을 처리하는 컨트롤러입니다.
 *
 * 주요 역할:
 * 1. 클라이언트로부터 참가자 준비 상태 변경 요청을 수신합니다.
 * 2. Principal 기반으로 사용자를 식별합니다.
...
public class GameRoomReadyController {
     ...
     * @param principal 인증된 사용자 정보 (Principal 기반)
    ...
    public void updateReadyStatus(
        ...
        try {
            // 준비 상태 변경 요청 수신 로그
            log.info("Ready status update request from user={} for room={}", principal.getName(), gameRoomId);
