...
// 1. [추가] apiClient와 authStore를 정적 임포트로 변경합니다.
        ...
        const offMsg = wsOnMessage(async (m) => {
            if (m && m.type === 'learning_ack') {
                ...
                if (m.status === 'accepted') {
                    ...
                    // 🎁 10점 지급 API 호출
                    try {
                        // 2. [수정] await import() 구문을 제거하고, 상단에서 가져온 모듈을 바로 사용합니다.
                        // const apiClient = (await import('../../services/api/apiClient.js')).default; // (제거)
                        await apiClient.post('/users/me/learning-reward');

                        // 3. [수정] await import() 구문을 제거하고, 상단에서 가져온 모듈을 바로 사용합니다.
                        // const { refreshMeSilent } = (await import('../../store/auth/authStore.js')).useAuthStore.getState(); // (제거)
